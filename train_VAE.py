...
Example cmd:
python train_VAE.py --state-dim 3 --weight\
     --expert-traj-path /home/jingjia/iliad/logs/data/infeasible_traj_3_with_weights.pkl \
         --size-per-traj 500 --output-path /home/jingjia/iliad/logs/debug
"""
import argparse
import os
import sys
...
def train_vae(args, dtype=torch.float32):
    ...
    state_pairs = generate_pairs(expert_traj_raw, state_dim, args.size_per_traj, max_step=10, min_step=5) # tune the step size if needed.
    # shuffle and split
    ...
    if args.weight:
        ...
        s, t = state_tuples[:, :state_dim-1], state_tuples[:, state_dim:2*state_dim]

        state_tuples_test = torch.from_numpy(test_state_tuples).to(dtype)
        s_test, t_test = state_tuples_test[:, :state_dim-1], state_tuples_test[:, state_dim:2 * state_dim]
    else:
        state_tuples = torch.from_numpy(state_pairs).to(dtype)
    ...
    for i in range(1, args.iter + 1):
        ...
        if args.weight:
            val_error =( t_test[:,-1].unsqueeze(1)*(t_test[:,:-1] - next_states) ** 2).mean()
        else:
